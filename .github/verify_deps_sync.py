...
DOCKERFILES = ["backend/Dockerfile", "backend/Dockerfile.test"]
...
def main():
    ...
    for dockerfile in DOCKERFILES:
        dockerfile_content_ = Path(dockerfile).read_text()
        uv_version = re.search(r"ghcr\.io/astral-sh/uv:(\d+\.\d+\.\d+)", dockerfile_content_).group(1)
